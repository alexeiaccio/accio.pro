...
class AccioSVG extends Component {
  ...
  updateWindowDimensions = () => {
    this.setState({
      isAnimate: this.state.isVertical !== (window.innerHeight / window.innerWidth > 1 ? true : false) && true,
    })
    setTimeout(() =>
      this.setState({
        isAnimate: false,
  ...
  keyframe = () => {
    this.setState({ animation: (this.state.animation + 2) % 360 })
  }

  render() {
    const style = {
      animation: spring(this.state.animation)
    }

    return (
      <SVGWrapper
        className={this.state.mount && 'mount'}
        style={{background: this.state.isVertical && '#000'}}
        >
        <Motion
          defaultStyle={{ animation: 0 }}
          style={style}
        >
        {value =>
          <StyledSVG viewBox='0 0 1440 1024' version='1.1' xmlns='http://www.w3.org/2000/svg'
            className={this.state.isVertical && 'vertical'}
            style={{opacity: this.state.isAnimate && 0}}
            >
            <g id='Canvas' transform='translate(967 150)'>
              ...
              <g id='accio' className='letters' transform='translate(0, 340)'>
                ...
                <path id='path4_fill' transform='translate(-285 29)' d='M 169.497 169.516L 273.982 303.735C 260.36 314.572 244.881 323.241 227.544 329.743C 210.207 335.935 190.858 339.032 169.497 339.032C 146.278 339.032 124.298 334.542 103.556 325.563C 83.1232 316.584 65.1674 304.509 49.6882 289.338C 34.5186 273.857 22.4448 255.899 13.4669 235.464C 4.48896 214.72 0 192.737 0 169.516C 0 146.294 4.48896 124.466 13.4669 104.032C 22.4448 83.2872 34.5186 65.3294 49.6882 50.1581C 65.1674 34.6772 83.1232 22.4473 103.556 13.4684C 124.298 4.48945 146.278 0 169.497 0C 187.143 0 204.325 2.78656 221.043 8.35968C 237.76 13.6232 253.24 21.3636 267.48 31.581L 169.497 169.516Z' />
            ...
            <defs>
              ...
              <filter id='hue'>
                <feColorMatrix in='SourceGraphic'
                  type='hueRotate'
                  values={ value.animation } />
              </filter>
              <filter id='blur'>
                <feGaussianBlur in='SourceGraphic' stdDeviation={ value.animation / 10 + 20 } />
              </filter>
            </defs>
          </StyledSVG>
        }
